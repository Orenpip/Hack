...
async function exportCoursesToCSV() {
  ...
  // Extract text from uploaded documents (if available)
  const resumeText = files.resume ? `Resume uploaded: ${files.resume.name}` : 'No resume uploaded';
  const transcriptText = files.transcript ? `Transcript uploaded: ${files.transcript.name}` : 'No transcript uploaded';
